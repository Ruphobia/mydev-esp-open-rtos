 ...
 * We need to default to debug messages off
 * or we get some stuff about the compiler and
 * what not before the user_init().
 * Unfortunatly there is some stuff that
 * happens during boot this is hard
 * coded into the chip that we cant silence :(
 */
int ets_printf_enabled = 0;
